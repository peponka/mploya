...
//
// Uso:
//   final compressed = await ImageCompressor.compressFile(File('path.jpg'));
//   await supabase.storage.from('avatars').uploadBinary('path', compressed);
// ─────────────────────────────────────────────────────────────────────────────
...
class ImageCompressor {
  ...
  /// Comprime un archivo de imagen. Retorna los bytes comprimidos.
  /// Se ejecuta en un isolate separado para no congelar la UI.
  static Future<Uint8List?> compressFile(File file, {int? maxW, int? quality}) async {
    try {
      final bytes = await file.readAsBytes();
      return compute(_compressBytes, _CompressParams(
        bytes: bytes,
        maxWidth: maxW ?? maxWidth,
        quality: quality ?? jpegQuality,
      ));
    } catch (e) {
      debugPrint('⚠️ ImageCompressor.compressFile: $e');
      return null;
    }
  }
