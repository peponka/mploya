...
class SocialAlgorithmService {
  ...
  List<Map<String, dynamic>> rankFeed(
    ...
    final withDiscovery = _injectDiscovery(diversified);

    // ── Cap de boosteados (salud del marketplace) ──
    return _capBoostedDensity(withDiscovery);
  }

  /// Evita que el feed se vuelva "pay-to-win". Los perfiles con boost pago siguen
  /// destacados (el primer lugar sigue siendo para un boost) pero quedan espaciados
  /// a ~1 cada [stride] posiciones, para que los candidatos orgánicos no se vuelvan
  /// invisibles y el feed no se vacíe de contenido genuino.
  List<Map<String, dynamic>> _capBoostedDensity(
    List<Map<String, dynamic>> users, {
    int stride = 4,
  }) {
    if (users.length <= stride) return users;

    bool isBoosted(Map<String, dynamic> u) {
      final raw = u['boost_ends_at'];
      if (raw == null) return false;
      final dt = DateTime.tryParse(raw.toString());
      return dt != null && dt.isAfter(DateTime.now());
    }

    final boosted = users.where(isBoosted).toList();
    if (boosted.isEmpty) return users;
    final organic = users.where((u) => !isBoosted(u)).toList();
    // Si casi todos están boosteados no hay nada orgánico con qué intercalar.
    if (organic.length <= boosted.length) return users;

    final result = <Map<String, dynamic>>[];
    int bi = 0, oi = 0, pos = 0;
    while (bi < boosted.length || oi < organic.length) {
      // Slot reservado para boost cada `stride` posiciones (0, 4, 8…).
      if (pos % stride == 0 && bi < boosted.length) {
        result.add(boosted[bi++]);
      } else if (oi < organic.length) {
        result.add(organic[oi++]);
      } else {
        result.add(boosted[bi++]); // se acabaron los orgánicos
      }
      pos++;
    }
    return result;
